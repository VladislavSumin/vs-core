...
import ru.vladislavsumin.core.ksp.utils.getAnnotation
    ...
    private fun generateFactory(
        ...
        val factoryInterface =
            (annotation.arguments.find { it.name?.asString() == "factoryInterface" }?.value as? KSType)
                ?.toClassName()
                .let { if (it == Types.Kotlin.Any) null else it }
        ...
        val factoryConstructorParams = if (factoryInterface == null) {
            constructorParams.filter { constructorParam ->
        ...
        val returnCodeBlock = CodeBlock.builder()
            ...
            .apply { constructorParams.forEach { parameter -> add("%L, ", parameter.name!!.getShortName()) } }
            .add(")")
            .build()
        ...
        TypeSpec.classBuilder(name)
            ...
            .addModifiers(annotation.visibilityModifier().toModifier())
