...
public inline fun <reified Ctx : GenericComponentContext<Ctx>> Modules.coreNavigation(): DI.Module =
    DI.Module("core-navigation") {
        ...
        bindSet<GenericNavigationRegistrar<Ctx, GenericComposeScreen<Ctx>>>()

        // Я не нашел, как нормально разорвать цикл зависимостей в kodein, поэтому пришлось добавить такой костыль.
        var navigation: GenericNavigation<Ctx, GenericComposeScreen<Ctx>>? = null

        bindSingleton { GenericNavigation<Ctx, GenericComposeScreen<Ctx>>(registrars = i()).also { navigation = it } }
